...
class NODE:
    ...
    def manager(self):
        ...
        for node in self.child:
            node.ucb = ((node.sum_value / node.nb_try) / 50 + node.proba) * \
                (math.log(self.nb_use) / (1 + 2 * math.pow(node.nb_use, 2)))
        ...
        if self.depth == -1 or (isWin(self.pos, self.board) == 0 and isFull(self.board) == 0):
            ...
            for i in range(nb_cell):
                if is_intresting(int(i % nb_row), int(i / nb_row), self.board) == 1:
                    self.create_child(
                        int(i / nb_row), int(i % nb_row), proba[i])
        else:
            self.simulate()
    ...
    def printv(self):
        for i in range(0, self.depth+2):
            sys.stdout.write('-')
        sys.stdout.flush()
        print(self.depth, self.nb_use, len(self.child), self.sum_value,
              self.nb_try, self.pos.y, self.pos.x, self.proba, self.ucb, self.printval)
        if self.depth < 0:
...
def get_best_move(root):
    ...
    for child in root.child:
        if child.sum_value != 0:
            ...
            target.append((child.nb_use / (root.nb_use - 1) * (child.proba * 1.5)))
...
def copy_model_parameters(sess, dqn1, dqn2):
    dqn1_params = [t for t in tf.trainable_variables()
                   if t.name.startswith(dqn1.scope)]
    dqn1_params = sorted(dqn1_params, key=lambda v: v.name)
    dqn2_params = [t for t in tf.trainable_variables()
                   if t.name.startswith(dqn2.scope)]
...
class QAI:
    def __init__(self, scope):
        ...
        with tf.variable_scope(scope):
            self.input = tf.placeholder(
                tf.float32, shape=(None, 1200), name='input')
            self.target = tf.placeholder(
                tf.float32, shape=(None, 400), name='target')
            self.layer = self.input
            self.layer = tf.layers.dense(self.input, 10800, activation=tf.nn.relu,
                                         kernel_initializer=tf.contrib.layers.variance_scaling_initializer(), name=None)
            self.qv = tf.layers.dense(self.layer, 400, activation=None,
                                      kernel_initializer=tf.contrib.layers.variance_scaling_initializer(), name='qv')
            self.prob = tf.nn.softmax(self.qv, name='prob')
            self.loss = tf.losses.mean_squared_error(
                predictions=self.qv, labels=self.target)
            self.train = tf.train.GradientDescentOptimizer(
                learning_rate=0.05).minimize(self.loss, name='train')
    ...
    def addMove(self, board, move):
        move = int(move)
        probas, qvs = self.sess.run(
            [self.prob, self.qv], feed_dict={self.input: [board]})
        lqv = qvs[0]
    ...
    def trainNeural(self, boardList, targetList, moveList):
        self.sess.run([self.train], feed_dict={
            self.input: boardList, self.target: targetList})
        boardList = []
        targetList = []
        moveList = []
    ...
    def learn(self, win):
        ...
        while i != len(self.moveList):
            ...
            i = i + 1
        self.sess.run([self.train], feed_dict={
            self.input: self.boardList, self.target: targetList})
...
graphicBoard = [[None]*20 for _ in range(20)]
...
def on_click(i, j, event):
    ...
    if graphicBoard[i][j]:
        return
    move = i * 20 + j
    game.placeMove(move, 2)
    graphicBoard[i][j] = "white"
    if game.isWin(move, 2) == 1:
        qai.reset()
        game.resetBoard()
        mt = 200
        graphicBoard = [[None]*20 for _ in range(20)]
        for i, row in enumerate(graphicBoard):
            for j, column in enumerate(row):
                L = tk.Label(root, text='    ', bg='brown')
                L.grid(row=i, column=j)
                L.bind('<Button-1>', lambda e, i=i, j=j: on_click(i, j, e))
        return
    game.printBoard()
    move = my_mcts(game.getBoard(), qai, 1, nbGame + 1, mt, 1)
    print(move)
    i = int(move / 20)
    j = int(move % 20)
    print(i)
    ...
    game.placeMove(move, 1)
    if game.isWin(move, 1) == 1:
        qai.reset()
        game.resetBoard()
        mt = 200
        graphicBoard = [[None]*20 for _ in range(20)]

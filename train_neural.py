...
if __name__ == '__main__':
    ...
    while i != nbGame:
        ...
        win = 0
        start = 0
        while w in == 0 and len(game.getMove()) != 0:
            ...
            if player == 1:
                if start == 0:
                    ...
                    move = qai.optiFive(game.convertBoard(), game.getInvMove(), game.getBoard())
                ...
                if start == 0:
                    ...
                    move = qai_copy.optiFive(game.convertBoardEnnemy(), game.getInvMove(), game.getBoard())
                game.placeMove(move, player)
                win = game.isWin(move, player)
                if win == 1:
                    win = 2
        ...
        if win == 1:
            winrate = winrate + 1
        boardList, targetList, moveList = qai.getGameList(win, boardList, targetList, moveList)
        qai.reset()
        game.resetBoard()
        while len(boardList)>= size_buffer:
            boardList, targetList, moveList = qai.trainNeural(boardList, targetList, moveList)
